...
@RestController
@Tag(name = "[Wayline] 웨이라인 작업", description = "웨이라인 작업 관리 API")
public class WaylineJobController {
     ...
     */
    @Operation(summary = "비행 작업 생성",
            description = "Dock을 위한 웨이라인 기반 비행 작업을 생성하고 발행합니다.")
    @PostMapping("/{workspace_id}/flight-tasks")
    public HttpResultResponse createJob(
            @Parameter(description = "워크스페이스 ID") @PathVariable("workspace_id") String workspaceId,
            @Valid @RequestBody CreateJobParam param,
     ...
     */
    @Operation(summary = "웨이라인 작업 목록 조회",
            description = "조건 및 페이징 기준으로 웨이라인 작업 목록을 조회합니다.")
    @GetMapping("/{workspace_id}/jobs")
    public HttpResultResponse<PaginationData<WaylineJobDTO>> getJobs(
            @Parameter(description = "조회할 페이지 번호 (기본값: 1)") @RequestParam(defaultValue = "1") Long page,
            @Parameter(description = "페이지당 항목 수 (기본값: 10)") @RequestParam(name = "page_size", defaultValue = "10") Long pageSize,
     ...
     * @throws SQLException 데이터베이스 작업 중 발생할 수 있는 예외
     */
    @Operation(summary = "웨이라인 작업 취소",
            description = "주어진 작업 ID 목록에 해당하는 웨이라인 작업을 취소합니다.")
    @DeleteMapping("/{workspace_id}/jobs")
    public HttpResultResponse publishCancelJob(
            @Parameter(description = "취소할 웨이라인 작업 ID 집합") @RequestParam(name = "job_id") Set<String> jobIds,
            @Parameter(description = "워크스페이스 ID") @PathVariable(name = "workspace_id") String workspaceId
    ) throws SQLException {
     ...
     */
    @Operation(summary = "미디어 우선 업로드 설정",
            description = "특정 웨이라인 작업의 미디어 파일을 우선 업로드로 설정합니다.")
    @PostMapping("/{workspace_id}/jobs/{job_id}/media-highest")
    public HttpResultResponse uploadMediaHighestPriority(
            @Parameter(description = "워크스페이스 ID") @PathVariable(name = "workspace_id") String workspaceId,
            @Parameter(description = "미디어 우선 업로드를 설정할 웨이라인 작업 ID") @PathVariable(name = "job_id") String jobId
    ) {
     ...
     */
    @Operation(summary = "웨이라인 작업 상태 업데이트",
            description = "특정 웨이라인 작업의 상태를 업데이트합니다.")
    @PutMapping("/{workspace_id}/jobs/{job_id}")
    public HttpResultResponse updateJobStatus(
            @Parameter(description = "워크스페이스 ID") @PathVariable(name = "workspace_id") String workspaceId,
            @Parameter(description = "업데이트할 웨이라인 작업 ID") @PathVariable(name = "job_id") String jobId,
            @Parameter(description = "작업 업데이트 파라미터 (상태, 진행률 등)") @Valid @RequestBody UpdateJobParam param
    ) {
        // 웨이라인 작업 상태 업데이트
