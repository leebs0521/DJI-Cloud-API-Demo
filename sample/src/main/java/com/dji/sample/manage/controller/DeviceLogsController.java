...
public class DeviceLogsController {
     ...
     */
    @Operation(summary = "업로드된 로그 목록 조회",
            description = "디바이스에서 업로드된 로그 파일들의 목록을 페이지네이션으로 조회합니다.")
    @GetMapping("/{workspace_id}/devices/{device_sn}/logs-uploaded")
    public HttpResultResponse getUploadedLogs(
            @Parameter(description = "로그 조회 파라미터") DeviceLogsQueryParam param,
     ...
     * @return 실시간 로그 파일 목록
     */
    @Operation(summary = "실시간 로그 파일 목록 조회",
            description = "디바이스에서 현재 업로드 가능한 로그 파일들의 목록을 조회합니다.")
    @GetMapping("/{workspace_id}/devices/{device_sn}/logs")
    public HttpResultResponse getLogsBySn(
            @Parameter(description = "워크스페이스 ID") @PathVariable("workspace_id") String workspaceId,
            @Parameter(description = "디바이스 시리얼 번호") @PathVariable("device_sn") String deviceSn,
            @Parameter(description = "로그 조회 파라미터") DeviceLogsGetParam param) {
     ...
     * @return 로그 업로드 요청 결과
     */
    @Operation(summary = "로그 업로드 요청",
            description = "디바이스에 로그 파일 업로드 명령을 전송합니다.")
    @PostMapping("/{workspace_id}/devices/{device_sn}/logs")
    public HttpResultResponse uploadLogs(
            @Parameter(description = "워크스페이스 ID") @PathVariable("workspace_id") String workspaceId,
            @Parameter(description = "디바이스 시리얼 번호") @PathVariable("device_sn") String deviceSn,
            @Parameter(description = "HTTP 요청", hidden = true) HttpServletRequest request,
            @Parameter(description = "로그 업로드 생성 파라미터") @RequestBody DeviceLogsCreateParam param
    ) {
     ...
     * @return 로그 업로드 취소 결과
     */
    @Operation(summary = "로그 업로드 취소",
            description = "진행 중인 로그 파일 업로드를 취소합니다.")
    @DeleteMapping("/{workspace_id}/devices/{device_sn}/logs")
    public HttpResultResponse cancelUploadedLogs(
            @Parameter(description = "워크스페이스 ID") @PathVariable("workspace_id") String workspaceId,
            @Parameter(description = "디바이스 시리얼 번호") @PathVariable("device_sn") String deviceSn,
            @Parameter(description = "파일 업로드 업데이트 요청") @RequestBody FileUploadUpdateRequest param) {
     ...
     * @return 업로드 이력 삭제 결과
     */
    @Operation(summary = "업로드 이력 삭제",
            description = "특정 로그 파일의 업로드 이력을 삭제합니다.")
    @DeleteMapping("/{workspace_id}/devices/{device_sn}/logs/{logs_id}")
    public HttpResultResponse deleteUploadedLogs(
            @Parameter(description = "워크스페이스 ID") @PathVariable("workspace_id") String workspaceId,
            @Parameter(description = "디바이스 시리얼 번호") @PathVariable("device_sn") String deviceSn,
            @Parameter(description = "로그 ID") @PathVariable("logs_id") String logsId) {
     ...
     */
    @Operation(summary = "로그 파일 다운로드 URL 조회",
            description = "로그 파일의 다운로드 URL을 생성하고 반환합니다.")
    @GetMapping("/{workspace_id}/logs/{logs_id}/url/{file_id}")
    public HttpResultResponse getFileUrl(
            ...
            @Parameter(description = "로그 ID") @PathVariable(name = "logs_id") String logsId,
