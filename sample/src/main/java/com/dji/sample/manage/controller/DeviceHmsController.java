...
public class DeviceHmsController {
     ...
     */
    @Operation(summary = "HMS 정보 페이지네이션 조회",
            description = "디바이스의 HMS 정보를 페이지네이션으로 조회합니다. 디바이스 시리얼 번호, 시간 범위, 메시지 타입 등으로 필터링할 수 있습니다.")
    @GetMapping("/{workspace_id}/devices/hms")
    public HttpResultResponse<PaginationData<DeviceHmsDTO>> getHmsInformation(
            @Parameter(description = "HMS 조회 파라미터") DeviceHmsQueryParam param,
     ...
     */
    @Operation(summary = "HMS 메시지 읽음 상태 업데이트",
            description = "특정 디바이스의 읽지 않은 HMS 메시지들을 읽음 상태로 변경합니다.")
    @PutMapping("/{workspace_id}/devices/hms/{device_sn}")
    public HttpResultResponse updateReadHmsByDeviceSn(
     ...
     * @return HMS 메시지 목록
     */
    @Operation(summary = "단일 디바이스 HMS 메시지 조회",
            description = "특정 디바이스의 HMS 메시지들을 조회합니다. 업데이트 시간을 기준으로 필터링하여 최신 메시지만 조회할 수 있습니다.")
    @GetMapping("/{workspace_id}/devices/hms/{device_sn}")
    public HttpResultResponse<List<DeviceHmsDTO>> getUnreadHmsByDeviceSn(
            @Parameter(description = "디바이스 시리얼 번호") @PathVariable("device_sn") String deviceSn
    ) {
