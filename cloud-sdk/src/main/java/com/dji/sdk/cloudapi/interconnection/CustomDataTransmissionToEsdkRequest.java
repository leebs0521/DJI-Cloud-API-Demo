...
/**
 * 클라우드에서 ESDK로의 커스텀 데이터 전송 요청 클래스
 * 
 * 이 클래스는 DJI Cloud API에서 클라우드에서 ESDK(Enterprise SDK)로 전송되는
 * 커스텀 데이터 요청을 담는 클래스입니다. 클라우드에서 ESDK 애플리케이션으로
...
public class CustomDataTransmissionToEsdkRequest extends BaseModel {
     ...
     * @return 현재 CustomDataTransmissionToEsdkRequest 객체
     */
